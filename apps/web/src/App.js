...
import { Users, Calendar, DollarSign, Activity, Shield, Plus, Search, Filter, Check, X, Trash2, Settings, RefreshCw, MessageSquare, UserPlus, Clock, AlertTriangle, Sun, Moon, Info, Lock } from 'lucide-react';
// ==========================================
...
export default function App() {
    ...
    const [theme, setTheme] = useState(() => {
        return localStorage.getItem('zenshin-theme') || 'dark';
    });
    // User auth state
    const [currentSession, setCurrentSession] = useState(() => {
        return loadFromStorage('zenshin-session', {
            id: 'U001',
    ...
    // DB States
    const [students, setStudents] = useState(() => loadFromStorage('zenshin-students', INITIAL_STUDENTS));
    const [trials, setTrials] = useState(() => loadFromStorage('zenshin-trials', INITIAL_TRIALS));
    const [ledger, setLedger] = useState(() => loadFromStorage('zenshin-ledger', INITIAL_LEDGER));
    const [auditLogs, setAuditLogs] = useState(() => loadFromStorage('zenshin-audit', INITIAL_AUDIT));
    const [timeline, setTimeline] = useState(() => loadFromStorage('zenshin-timeline', INITIAL_TIMELINE));
    const [settings, setSettings] = useState(() => loadFromStorage('zenshin-settings', INITIAL_SETTINGS));
    // Router simulation
    const [currentTab, setCurrentTab] = useState('dashboard');
    // Filter and UI States
    const [activeBranch, setActiveBranch] = useState('Sirifort');
    ...
    const [billingType, setBillingType] = useState('PAYMENT');
    ...
    const [batchAttendanceState, setBatchAttendanceState] = useState({});
    ...
    const logAudit = (action, details, branch) => {
        const newLog = {
    ...
    const logTimeline = (studentId, type, description) => {
        const newEvent = {
    ...
    const canPerform = (requiredRoles) => {
    ...
    const getFilteredStudents = () => {
        ...
        return students.filter(s => {
            ...
            if (searchQuery) {
                ...
                if (!matchesName && !matchesId && !matchesParent && !matchesPhone)
                    return false;
            }
            // Belt Filter
            if (beltFilter !== 'All' && s.currentBelt !== beltFilter)
                return false;
            // Status Filter
    ...
    const generateStudentId = (branch) => {
    ...
    const openBillingModal = (student, type) => {
    ...
    const handleAddLedgerEntry = (e) => {
        ...
        setStudents(updatedStudents);
        logAudit(billingType === 'CHARGE' ? 'CHARGE_LOGGED' : 'PAYMENT_RECEIVED', `${billingType} of ₹${billingAmount} recorded for ${student.id} (${billingDesc})`, student.branch);
        logTimeline(student.id, billingType === 'CHARGE' ? 'CHARGE_LOGGED' : 'PAYMENT_RECEIVED', `${billingType} of ₹${billingAmount} recorded: ${billingDesc}`);
    ...
    const submitBatchAttendance = () => {
        ...
        logAudit('ATTENDANCE_MARKED', `Batch Attendance submitted for ${attendanceBatch} on ${attendanceDate}. Present: ${presentCount}, Late: ${lateCount}, Absent: ${absentCount}`, activeBranch);
    ...
    const handleUpdateSettings = (e) => {
        ...
        const formData = new FormData(e.currentTarget);
        const newSettings = {
            maxGracePeriod: parseInt(formData.get('maxGracePeriod')) || 10,
            reactivationCharge: parseInt(formData.get('reactivationCharge')) || 1000
        };
    ...
    return (<div className="min-h-screen text-slate-100 flex flex-col antialiased">
      
      {/* ==========================================
            WHATSAPP FLOATING BANNER (SIMULATION)
           ========================================== */}
      {whatsappToast.visible && (<div className="fixed top-6 right-6 z-50 max-w-md w-full glass-card border border-emerald-500/30 bg-emerald-950/80 p-4 rounded-xl shadow-2xl transition-all duration-300 transform translate-y-0 scale-100 animate-pulse">
      ...
      <header className="border-b border-[var(--border-muted)] bg-[var(--bg-secondary)] py-4 px-6 sticky top-0 z-40 transition-all duration-300">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {currentSession.role === 'OWNER' && (<button onClick={() => {
            ...
            }} className="p-2.5 rounded-lg border border-[var(--border-muted)] hover:bg-[var(--bg-tertiary)] text-[var(--text-muted)] transition-all hover:text-[var(--text-primary)]" title="System settings">
            ...
            <div className="flex items-center gap-1.5 border border-[var(--border-muted)] bg-[var(--bg-tertiary)] p-1 rounded-lg">
              ...
              {['OWNER', 'MANAGER', 'INSTRUCTOR', 'PARENT'].map(r => (<button key={r} onClick={() => switchUserSession(r)} className={`px-2 py-1 rounded text-[10px] font-bold font-mono transition-all ${currentSession.role === r
            ...
            <div className="flex items-center gap-1 bg-[var(--bg-tertiary)] p-1 rounded-lg border border-[var(--border-muted)]">
              {['Sirifort', 'Asiad'].map(b => {
            const isLocked = currentSession.role === 'MANAGER' && currentSession.branch !== b;
            return (<button key={b} disabled={isLocked} onClick={() => setActiveBranch(b)} className={`px-3 py-1 rounded-md text-xs font-semibold flex items-center gap-1.5 transition-all ${activeBranch === b
                    ? 'bg-[var(--bg-primary)] text-[var(--text-primary)] border border-[var(--border-muted)]'
                    : 'text-[var(--text-muted)] hover:text-[var(--text-primary)]'} ${isLocked ? 'opacity-30 cursor-not-allowed' : ''}`}>
                    <span>{b} Dojo</span>
            ...
            {/* Theme Toggle */}
            <button onClick={() => setTheme(prev => prev === 'dark' ? 'red' : 'dark')} className="p-2.5 rounded-lg border border-[var(--border-muted)] hover:bg-[var(--bg-tertiary)] text-[var(--text-muted)] transition-all hover:text-[var(--text-primary)]">
      ...
      <nav className="bg-[var(--bg-tertiary)] border-b border-[var(--border-muted)] px-6">
        <div className="max-w-7xl mx-auto flex overflow-x-auto scrollbar-none gap-6">
          {[
            ...
            return (<button key={tab.id} onClick={() => setCurrentTab(tab.id)} className={`py-4 px-1 flex items-center gap-2 border-b-2 font-semibold text-sm whitespace-nowrap transition-all duration-300 ${isActive
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full p-6 space-y-6">
        ...
        {stats.overdueCount > 0 && (<div className="p-4 rounded-xl border border-amber-500/20 bg-amber-950/20 flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-3">
            ...
            <button onClick={runCronJobSimulation} className="px-4 py-2 rounded-lg bg-amber-600 hover:bg-amber-700 text-slate-950 text-xs font-bold transition-all shadow-lg shadow-amber-500/10 flex items-center gap-2">
        ...
        {currentTab === 'dashboard' && (<div className="space-y-6 animate-fadeIn">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="my-6 space-y-4">
                  <div>
                    ...
                    <div className="w-full bg-[var(--border-muted)] h-3 rounded-full overflow-hidden">
                      <div className="bg-gradient-to-r from-amber-500 to-red-500 h-full rounded-full transition-all" style={{ width: `${Math.min(100, Math.round((stats.pendingFees / (stats.monthlyFees || 1)) * 100))}%` }}></div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="w-full bg-[var(--border-muted)] h-3 rounded-full overflow-hidden">
                      <div className="bg-gradient-to-r from-emerald-500 to-[var(--accent-secondary)] h-full rounded-full transition-all" style={{ width: `${Math.min(100, Math.round((trials.filter(t => t.status === 'JOINED').length / (trials.length || 1)) * 100))}%` }}></div>
                    </div>
                  </div>
                </div>
              ...
              <div className="glass-card p-6 rounded-2xl flex flex-col justify-between">
                ...
                <div className="my-4 divide-y divide-[var(--border-muted)] max-h-[160px] overflow-y-auto scrollbar-thin">
                  {students.filter(s => s.status === 'INACTIVE' && s.branch === activeBranch).map(student => (<div key={student.id} className="py-2.5 flex items-center justify-between text-xs">
                      <div>
                  ...
                  {students.filter(s => s.status === 'INACTIVE' && s.branch === activeBranch).length === 0 && (<div className="text-center py-6 text-xs text-[var(--text-muted)]">
                      No suspended members in {activeBranch} dojo. Excellent work!
                    </div>)}
                </div>

                <button onClick={() => setCurrentTab('billing')} className="w-full py-2 bg-[var(--border-muted)] hover:bg-[var(--border-glow)] rounded-lg text-xs font-semibold transition-all text-center border border-[var(--border-muted)]">
            ...
            <div className="glass-card p-6 rounded-2xl">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <button onClick={() => setShowAddStudent(true)} className="p-4 rounded-xl border border-[var(--border-muted)] bg-[var(--bg-tertiary)] hover:border-[var(--accent-primary)] hover:bg-[var(--border-glow)] text-center transition-all group">
                ...
                <button onClick={() => setShowAddTrial(true)} className="p-4 rounded-xl border border-[var(--border-muted)] bg-[var(--bg-tertiary)] hover:border-[var(--accent-secondary)] hover:bg-cyan-500/10 text-center transition-all group">
                ...
                <button onClick={runCronJobSimulation} className="p-4 rounded-xl border border-[var(--border-muted)] bg-[var(--bg-tertiary)] hover:border-amber-500 hover:bg-amber-500/10 text-center transition-all group">
        ...
        {currentTab === 'students' && (<div className="space-y-6 animate-fadeIn">
            ...
            <div className="glass-card p-4 rounded-2xl flex flex-col md:flex-row gap-4 items-center justify-between">
              ...
              <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
                ...
                <div className="flex items-center gap-1.5 text-xs">
                  ...
                  <select value={beltFilter} onChange={(e) => setBeltFilter(e.target.value)} className="bg-[var(--bg-tertiary)] border border-[var(--border-muted)] rounded-lg py-1.5 px-3 text-xs text-[var(--text-primary)] focus:outline-none">
                ...
                {/* Status filter */}
                <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="bg-[var(--bg-tertiary)] border border-[var(--border-muted)] rounded-lg py-1.5 px-3 text-xs text-[var(--text-primary)] focus:outline-none">
                ...
                {canPerform(['OWNER', 'MANAGER']) && (<button onClick={() => setShowAddStudent(true)} className="ml-auto px-4 py-2 bg-gradient-to-r from-[var(--accent-primary)] to-[var(--accent-secondary)] text-slate-950 text-xs font-bold rounded-lg hover:scale-105 transition-all shadow-md flex items-center gap-1.5">
            ...
            <div className="glass-card rounded-2xl overflow-hidden border border-[var(--border-glow)]">
              <div className="overflow-x-auto">
                ...
                return (<tr key={student.id} className="hover:bg-[var(--bg-secondary)]/50 transition-all cursor-pointer" onClick={() => setSelectedStudent(student)}>
                          ...
                          <td className="py-3.5 px-5">
                            <div className="flex items-center gap-2">
                              <div className="w-12 bg-slate-800 h-1.5 rounded-full overflow-hidden">
                                <div className={`h-full ${student.attendanceRate >= 85 ? 'bg-emerald-500' : 'bg-amber-500'}`} style={{ width: `${student.attendanceRate}%` }}></div>
                              </div>
                              <span className="font-mono">{student.attendanceRate}%</span>
                            </div>
                          </td>
                          <td className="py-3.5 px-5">
                            {student.outstandingBalance > 0 ? (<div>
                                <span className={`px-2 py-0.5 rounded font-bold font-mono text-[10px] ${isOverdue ? 'bg-red-500/20 text-red-400 border border-red-500/20' : 'bg-amber-500/20 text-amber-300 border border-amber-500/20'}`}>
                                  ₹{student.outstandingBalance} unpaid
                                </span>
                                <p className="text-[9px] text-[var(--text-muted)] mt-1">Due: {student.feeDueDate}</p>
                              </div>) : (<span className="px-2 py-0.5 rounded text-[10px] font-bold bg-emerald-500/20 text-emerald-400 border border-emerald-500/20">
                                Paid Settled
                              </span>)}
                          </td>
                          <td className="py-3.5 px-5">
                            {student.status === 'ACTIVE' ? (<span className="px-2 py-0.5 rounded-full text-[10px] bg-emerald-500/10 text-emerald-400">
                                ACTIVE
                              </span>) : (<span className="px-2 py-0.5 rounded-full text-[10px] bg-red-500/10 text-red-400 font-bold border border-red-500/20">
                                SUSPENDED
                              </span>)}
                          </td>
                          <td className="py-3.5 px-5 text-center" onClick={(e) => e.stopPropagation()}>
                            <div className="flex items-center justify-center gap-1.5">
                              {/* Log Payment action */}
                              {canPerform(['OWNER', 'MANAGER']) && (<button onClick={() => openBillingModal(student, 'PAYMENT')} className="px-2.5 py-1 bg-emerald-600 hover:bg-emerald-700 text-slate-950 rounded text-[10px] font-bold transition-all" title="Log Payment">
                                  Pay
                                </button>)}
                              {/* Log Charge action */}
                              {canPerform(['OWNER', 'MANAGER']) && (<button onClick={() => openBillingModal(student, 'CHARGE')} className="px-2.5 py-1 bg-slate-800 hover:bg-[var(--border-glow)] border border-[var(--border-muted)] rounded text-[10px] font-bold text-[var(--text-primary)] transition-all" title="Add Charge">
                                  Charge
                                </button>)}
                            </div>
                          </td>
                    ...
                    {getFilteredStudents().length === 0 && (<tr>
        ...
        {currentTab === 'attendance' && (<div className="space-y-6 animate-fadeIn">
            ...
            <div className="glass-card p-5 rounded-2xl">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  {/* Select Date */}
                  <input type="date" value={attendanceDate} onChange={(e) => setAttendanceDate(e.target.value)} className="bg-[var(--bg-tertiary)] border border-[var(--border-muted)] rounded-lg py-1.5 px-3 text-xs text-[var(--text-primary)] focus:outline-none"/>

                  {/* Select Batch */}
                  <select value={attendanceBatch} onChange={(e) => setAttendanceBatch(e.target.value)} className="bg-[var(--bg-tertiary)] border border-[var(--border-muted)] rounded-lg py-1.5 px-3 text-xs text-[var(--text-primary)] focus:outline-none">
                  ...
                  <button onClick={submitBatchAttendance} className="px-5 py-2 bg-gradient-to-r from-emerald-500 to-[var(--accent-secondary)] text-slate-950 font-bold text-xs rounded-lg hover:scale-105 transition-all shadow-lg flex items-center gap-2">
            ...
            <div className="glass-card rounded-2xl p-6 border border-[var(--border-glow)]">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                return (<div key={student.id} className={`p-4 rounded-xl border transition-all flex items-center justify-between ${currentStatus === 'PRESENT'
                        ...
                        <div className="flex gap-1.5">
                          {/* Present Button */}
                          <button onClick={() => setBatchAttendanceState(prev => ({ ...prev, [student.id]: 'PRESENT' }))} className={`px-2 py-1 rounded text-[10px] font-bold transition-all ${currentStatus === 'PRESENT'
                        ? 'bg-emerald-500 text-slate-950 font-extrabold shadow'
                        : 'bg-[var(--bg-tertiary)] text-[var(--text-muted)] hover:bg-[var(--border-glow)]'}`}>
                            Present
                          </button>
                          {/* Late Button */}
                          <button onClick={() => setBatchAttendanceState(prev => ({ ...prev, [student.id]: 'LATE' }))} className={`px-2 py-1 rounded text-[10px] font-bold transition-all ${currentStatus === 'LATE'
                        ? 'bg-amber-500 text-slate-950 font-extrabold shadow'
                        : 'bg-[var(--bg-tertiary)] text-[var(--text-muted)] hover:bg-[var(--border-glow)]'}`}>
                            Late
                          </button>
                          {/* Absent Button */}
                          <button onClick={() => setBatchAttendanceState(prev => ({ ...prev, [student.id]: 'ABSENT' }))} className={`px-2 py-1 rounded text-[10px] font-bold transition-all ${currentStatus === 'ABSENT'
                ...
                {students.filter(s => s.branch === activeBranch && s.status === 'ACTIVE').length === 0 && (<div className="col-span-full py-12 text-center text-[var(--text-muted)] text-sm">
        ...
        {currentTab === 'trials' && (<div className="space-y-6 animate-fadeIn">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              {canPerform(['OWNER', 'MANAGER']) && (<button onClick={() => setShowAddTrial(true)} className="px-4 py-2 bg-gradient-to-r from-[var(--accent-primary)] to-[var(--accent-secondary)] text-slate-950 text-xs font-bold rounded-lg hover:scale-105 transition-all shadow-md flex items-center gap-1.5">
                  <Plus className="w-3.5 h-3.5 stroke-[3px]"/>
                  Add Trial Lead
                </button>)}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              ...
              <div className="glass-card p-4 rounded-xl border border-[var(--border-muted)] bg-[var(--bg-secondary)] flex flex-col gap-3 min-h-[300px]">
                ...
                <div className="flex-1 space-y-3 overflow-y-auto max-h-[400px] scrollbar-thin">
                  {trials.filter(t => t.branch === activeBranch && t.status === 'NEW').map(t => (<div key={t.id} className="p-3 rounded-lg bg-[var(--bg-tertiary)] border border-[var(--border-muted)] flex flex-col gap-2">
                      <div>
                      ...
                      <div className="flex justify-end gap-1">
                        <button onClick={() => updateTrialStatus(t.id, 'PAID')} className="px-2 py-1 bg-emerald-600/20 border border-emerald-500/20 text-emerald-400 rounded text-[9px] font-bold">
                          Collect ₹500
                        </button>
                      </div>
                    </div>))}
              ...
              <div className="glass-card p-4 rounded-xl border border-[var(--border-muted)] bg-[var(--bg-secondary)] flex flex-col gap-3 min-h-[300px]">
                ...
                <div className="flex-1 space-y-3 overflow-y-auto max-h-[400px] scrollbar-thin">
                  {trials.filter(t => t.branch === activeBranch && t.status === 'PAID').map(t => (<div key={t.id} className="p-3 rounded-lg bg-[var(--bg-tertiary)] border border-[var(--border-muted)] flex flex-col gap-2">
                      <div>
                        <p className="text-xs font-bold text-[var(--text-primary)]">{t.name}</p>
                        <p className="text-[9px] text-[var(--text-muted)] font-mono">Mob: {t.mobile}</p>
                      ...
                      <div className="flex justify-end gap-1">
                        <button onClick={() => updateTrialStatus(t.id, 'TRIAL_COMPLETED')} className="px-2 py-1 bg-amber-600/20 border border-amber-500/20 text-amber-400 rounded text-[9px] font-bold">
                          Trial Done
                        </button>
                      </div>
                    </div>))}
              ...
              <div className="glass-card p-4 rounded-xl border border-[var(--border-muted)] bg-[var(--bg-secondary)] flex flex-col gap-3 min-h-[300px]">
                ...
                <div className="flex-1 space-y-3 overflow-y-auto max-h-[400px] scrollbar-thin">
                  {trials.filter(t => t.branch === activeBranch && t.status === 'TRIAL_COMPLETED').map(t => (<div key={t.id} className="p-3 rounded-lg bg-[var(--bg-tertiary)] border border-[var(--border-muted)] flex flex-col gap-2">
                      <div>
                        <p className="text-xs font-bold text-[var(--text-primary)]">{t.name}</p>
                        <p className="text-[9px] text-[var(--text-muted)] font-mono">Mob: {t.mobile}</p>
                      </div>
                      <div className="flex gap-1 justify-end">
                        <button onClick={() => updateTrialStatus(t.id, 'JOINED')} className="px-2 py-1 bg-emerald-600 text-slate-950 rounded text-[9px] font-bold">
                          Convert
                        </button>
                        <button onClick={() => updateTrialStatus(t.id, 'LOST')} className="px-2 py-1 bg-red-600/20 border border-red-500/20 text-red-400 rounded text-[9px] font-bold">
                          Lost
                        </button>
                      </div>
                    </div>))}
              ...
              <div className="glass-card p-4 rounded-xl border border-[var(--border-muted)] bg-[var(--bg-secondary)] flex flex-col gap-3 min-h-[300px]">
                ...
                <div className="flex-1 space-y-3 overflow-y-auto max-h-[400px] scrollbar-thin">
                  {trials.filter(t => t.branch === activeBranch && t.status === 'JOINED').map(t => (<div key={t.id} className="p-3 rounded-lg bg-[var(--bg-tertiary)] border border-[var(--border-muted)] flex flex-col gap-2 opacity-70">
                      <div>
                        <p className="text-xs font-bold text-[var(--text-primary)]">{t.name}</p>
                        <p className="text-[9px] text-[var(--text-muted)] font-mono">Mob: {t.mobile}</p>
              ...
              <div className="glass-card p-4 rounded-xl border border-[var(--border-muted)] bg-[var(--bg-secondary)] flex flex-col gap-3 min-h-[300px]">
                ...
                <div className="flex-1 space-y-3 overflow-y-auto max-h-[400px] scrollbar-thin">
                  {trials.filter(t => t.branch === activeBranch && t.status === 'LOST').map(t => (<div key={t.id} className="p-3 rounded-lg bg-[var(--bg-tertiary)] border border-[var(--border-muted)] flex flex-col gap-2 opacity-70">
                      <div>
                        <p className="text-xs font-bold text-[var(--text-primary)]">{t.name}</p>
                        <p className="text-[9px] text-[var(--text-muted)] font-mono">Mob: {t.mobile}</p>
                      </div>
        ...
        {currentTab === 'billing' && (<div className="space-y-6 animate-fadeIn">
              ...
              <div className="glass-card p-6 rounded-2xl flex flex-col justify-between">
                ...
                <form onSubmit={handleUpdateSettings} className="my-6 space-y-4">
                  <div>
                    ...
                    <input type="number" name="maxGracePeriod" defaultValue={settings.maxGracePeriod} disabled={!canPerform(['OWNER'])} className="w-full bg-[var(--bg-tertiary)] border border-[var(--border-muted)] rounded-lg py-2 px-3 text-xs text-[var(--text-primary)] focus:outline-none focus:border-[var(--accent-primary)]"/>
                  ...
                  <div>
                    ...
                    <input type="number" name="reactivationCharge" defaultValue={settings.reactivationCharge} disabled={!canPerform(['OWNER'])} className="w-full bg-[var(--bg-tertiary)] border border-[var(--border-muted)] rounded-lg py-2 px-3 text-xs text-[var(--text-primary)] focus:outline-none focus:border-[var(--accent-primary)]"/>
                    <p className="text-[10px] text-[var(--text-muted)] mt-1">
                      Mandatory unlock penalty charged upon auto-suspension.
                    </p>
                  </div>

                  {canPerform(['OWNER']) && (<button type="submit" className="w-full py-2 bg-gradient-to-r from-[var(--accent-primary)] to-[var(--accent-secondary)] text-slate-950 font-bold text-xs rounded-lg hover:scale-[1.02] transition-all shadow">
                      Save Parameters
                    </button>)}
                </form>
            ...
            }).map(item => (<div key={item.id} className="py-3 flex justify-between items-center text-xs">
                      <div>
                        <div className="flex items-center gap-2">
        ...
        {currentTab === 'audit' && (<div className="space-y-6 animate-fadeIn">
            ...
            <div className="glass-card p-6 rounded-2xl border border-[var(--border-glow)]">
              ...
              <div className="space-y-3 overflow-y-auto max-h-[500px] scrollbar-thin">
                {auditLogs.map(log => (<div key={log.id} className="p-3.5 rounded-xl bg-[var(--bg-secondary)] border border-[var(--border-muted)] hover:border-[var(--border-glow)] transition-all">
                    ...
                    <div className="mt-2 flex items-center justify-between text-xs">
                      <div className="flex gap-2 items-center">
                        <span className={`px-2 py-0.5 rounded font-mono text-[9px] font-bold ${log.action.includes('SUSPEND') || log.action.includes('FAILED')
      ...
      {selectedStudent && (<div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-filter backdrop-blur-sm flex justify-end transition-all">
          <div className="w-full max-w-lg bg-[var(--bg-secondary)] border-l border-[var(--border-muted)] h-full overflow-y-auto p-6 flex flex-col justify-between shadow-2xl animate-slideLeft">
            ...
            <div className="space-y-6">
              ...
              <div className="flex justify-between items-start border-b border-[var(--border-muted)] pb-4">
                ...
                <button onClick={() => setSelectedStudent(null)} className="p-1.5 rounded-lg border border-[var(--border-muted)] hover:bg-[var(--bg-tertiary)] text-[var(--text-muted)] transition-all">
                  <X className="w-4 h-4"/>
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 bg-[var(--bg-tertiary)] p-4 rounded-xl border border-[var(--border-muted)] text-xs">
                ...
                <div>
                  ...
                  <p className={`font-bold mt-0.5 font-mono ${new Date(selectedStudent.feeDueDate) < new Date() && selectedStudent.outstandingBalance > 0 ? 'text-red-400' : 'text-[var(--text-primary)]'}`}>{selectedStudent.feeDueDate}</p>
                </div>
                <div>
              ...
              <div className="space-y-3">
                ...
                <div className="border-l border-[var(--border-muted)] pl-4 ml-2 space-y-4">
                  {timeline.filter(e => e.studentId === selectedStudent.id).map(e => (<div key={e.id} className="relative text-xs">
                      {/* Timeline dot */}
                  ...
                  {timeline.filter(e => e.studentId === selectedStudent.id).length === 0 && (<div className="text-center py-6 text-[var(--text-muted)] italic">
            ...
            <div className="border-t border-[var(--border-muted)] pt-4 flex gap-2">
              ...
              {selectedStudent.status === 'INACTIVE' && (<button onClick={() => {
              ...
              {canPerform(['OWNER', 'MANAGER']) && (<button onClick={() => {
                ...
                }} className="p-2.5 rounded-lg bg-red-950/20 border border-red-500/20 text-red-400 hover:bg-red-900/30 text-xs transition-all" title="Remove Student Profile">
      ...
      {showAddStudent && (<div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-filter backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-[var(--bg-secondary)] border border-[var(--border-muted)] rounded-2xl p-6 shadow-2xl animate-scaleUp">
            ...
            <div className="flex justify-between items-center border-b border-[var(--border-muted)] pb-3 mb-4">
              ...
              <button onClick={() => setShowAddStudent(false)} className="p-1 rounded-lg border border-[var(--border-muted)] hover:bg-[var(--bg-tertiary)] text-[var(--text-muted)] transition-all">
                <X className="w-4 h-4"/>
              </button>
            </div>
      ...
      {showAddTrial && (<div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-filter backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-[var(--bg-secondary)] border border-[var(--border-muted)] rounded-2xl p-6 shadow-2xl animate-scaleUp">
            
            <div className="flex justify-between items-center border-b border-[var(--border-muted)] pb-3 mb-4">
              <h3 className="text-base font-bold text-[var(--text-primary)]">Add Dojo Trial Lead</h3>
              <button onClick={() => setShowAddTrial(false)} className="p-1 rounded-lg border border-[var(--border-muted)] hover:bg-[var(--bg-tertiary)] text-[var(--text-muted)] transition-all">
                <X className="w-4 h-4"/>
              </button>
            </div>
      ...
      {showBillingModal && (<div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-filter backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-[var(--bg-secondary)] border border-[var(--border-muted)] rounded-2xl p-6 shadow-2xl animate-scaleUp">
            ...
            <div className="flex justify-between items-center border-b border-[var(--border-muted)] pb-3 mb-4">
              ...
              <button onClick={() => setShowBillingModal(false)} className="p-1 rounded-lg border border-[var(--border-muted)] hover:bg-[var(--bg-tertiary)] text-[var(--text-muted)] transition-all">
                <X className="w-4 h-4"/>
              </button>
            </div>
            ...
            <form onSubmit={handleAddLedgerEntry} className="space-y-4 text-xs">
              ...
              <div>
                ...
                <input type="text" disabled value={students.find(s => s.id === billingStudentId)?.name || ''} className="w-full bg-[var(--bg-tertiary)] border border-[var(--border-muted)] rounded-lg py-2 px-3 text-xs text-[var(--text-primary)] focus:outline-none opacity-60"/>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-bold text-[var(--text-muted)] block mb-1">Transaction Type</label>
                  <select value={billingType} onChange={(e) => setBillingType(e.target.value)} className="w-full bg-[var(--bg-tertiary)] border border-[var(--border-muted)] rounded-lg py-2 px-3 text-xs text-[var(--text-primary)] focus:outline-none focus:border-[var(--accent-primary)]">
                ...
                <div>
                  ...
                  <input type="number" required value={billingAmount} onChange={(e) => setBillingAmount(parseInt(e.target.value) || 0)} className="w-full bg-[var(--bg-tertiary)] border border-[var(--border-muted)] rounded-lg py-2 px-3 text-xs text-[var(--text-primary)] focus:outline-none focus:border-[var(--accent-primary)] font-mono font-bold"/>
                </div>
              </div>

              <div>
                <label className="text-[10px] font-bold text-[var(--text-muted)] block mb-1">Description / Memo</label>
                <input type="text" required value={billingDesc} onChange={(e) => setBillingDesc(e.target.value)} placeholder="e.g. Monthly Tuition Fee - June 2026" className="w-full bg-[var(--bg-tertiary)] border border-[var(--border-muted)] rounded-lg py-2 px-3 text-xs text-[var(--text-primary)] focus:outline-none focus:border-[var(--accent-primary)]"/>
              </div>
      ...
      {showReceipt && (<div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-filter backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-sm bg-slate-900 border border-slate-700 rounded-2xl p-6 shadow-2xl text-slate-200">
            ...
            <div className="mt-6 flex gap-2">
              ...
              <button onClick={() => setShowReceipt(null)} className="px-4 py-2 rounded-lg bg-slate-800 border border-slate-700 text-slate-400 hover:text-slate-200 text-xs font-bold transition-all">
                Close
              </button>
            </div>

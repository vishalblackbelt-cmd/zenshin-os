...
// GET /api/attendance
router.get('/', (0, auth_js_1.requireRole)(['OWNER', 'MANAGER', 'INSTRUCTOR']), async (req, res) => {
    ...
    try {
        ...
        if (branchName)
            whereClause.student = { branch: { name: branchName } };
        const logs = await db_js_1.prisma.attendance.findMany({
            where: whereClause,
            include: {
...
// POST /api/attendance
router.post('/', (0, auth_js_1.requireRole)(['OWNER', 'MANAGER', 'INSTRUCTOR']), auth_js_1.requireBranchAccess, async (req, res) => {
    ...
    try {
        ...
        const loggedRecords = await db_js_1.prisma.$transaction(async (tx) => {
            const results = [];

...
export async function runFinancialCron(): Promise<{ suspensions: number; friendlyReminders: number; overdueReminders: number }> {
  ...
  for (const student of activeStudents) {
    ...
    if (diffDays <= -settings.maxGracePeriod) {
      ...
      await sendWhatsAppMessage(student.mobile, suspensionMsg, student.branchId);
    ...
    // Case 2: Friendly Reminder (5 days before due date, diffDays === 5)
      ...
      await sendWhatsAppMessage(student.mobile, reminderMsg, student.branchId);
    } 
    // Case 3: Overdue Warning (5 days after due date, diffDays === -5)
      ...
      await sendWhatsAppMessage(student.mobile, warningMsg, student.branchId);
    }
  }
